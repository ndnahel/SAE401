...
class FavoriteCityController extends AbstractController
{
    ...
    public function addCity(
        ...
        if (!$user) {
            $this->addFlash(
                'error',
                $this->translator->trans('Vous devez être connecté pour ajouter une ville en favori.')
            );
            return $this->redirectToRoute('app_home');
        }
    ...
    public function removeCity(
        ...
        if (!$user) {
            $this->addFlash(
                'error',
                $this->translator->trans('Vous devez être connecté pour supprimer une ville en favori.')
            );
            return $this->redirectToRoute('app_home');
        }
